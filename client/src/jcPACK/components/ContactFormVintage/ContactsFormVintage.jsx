...
export default function ContactsFormVintage() {
    ...
    const [subjectInput, setSubjectInput] = useState("reservation");
    ...
    function handleSeatsChange(event){
        if(event.target.value.length < 3){
            setSeatsInput(event.target.value);
            
            if(errorInput === "os lugares" && event.target.value)
            setErrorInput("");
        }
    }
...
<div id="container">
  ...
  {!sentForm && <form ref={formRef} id="contact_form">
    <div className="name">
